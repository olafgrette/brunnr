...
**Search layer ([qmd](https://github.com/tobi/qmd), optional)** — a local retrieval index over the well (`<wellname>-wiki` over `wiki/`, `<wellname>-source` over `source/`), used by the playbooks at the moment they act. Setup is a deliberate, agent-run step (`procedures/qmd-setup.md`), not part of `brunnr-init`. The index and models live under `~/.cache/qmd/` — **machine-local, not synced** — so a synced well is set up once per machine. You don't call qmd directly; use the `brunnr` command (on PATH — `brunnr-init` symlinks it into `~/.local/bin`). Run it from inside the well; it resolves which well from your working directory:
- `brunnr search-enabled` — exits 0 if search is set up here (the gate every playbook branches on).
...
qmd is fully optional — if `search-enabled` is false, every playbook falls back to reading `index.md`. `index.md` stays the human-curated map; qmd is the machine retrieval index that finds the right pages without reading the whole map at scale, not a replacement for it. The index is refreshed at write/audit time (after ingest/sync, before lint) via `procedures/qmd-update.md`, not on every query.
