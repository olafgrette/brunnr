...
`bin/brunnr-init` installs a schema + playbooks into a **vault** (a directory with `raw/` sources and an LLM-maintained `wiki/`). A vault receives: `AGENTS.md` (from `vault-AGENTS.md`), a `CLAUDE.md` mirror, `procedures/`, Claude Code shims under `.claude/skills/`, and seeded `VAULT.md` + `wiki/{index,log}.md`.
