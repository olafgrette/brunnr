...
1. **Find the relevant pages.**
   - **If `brunnr enabled`**, search instead of scanning `index.md` by hand. The index is already current (refreshed at write time), so don't re-index — just search:
     - `brunnr keyword wiki "<question>"` — BM25, instant. The default, enough for most wells.
     - For paraphrase/semantic recall: `brunnr semantic "<question>"`, or hybrid `brunnr query "<question>"` (slower, reranked).
