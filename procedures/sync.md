...
1. **Diff the Source:** 
   - Read the frontmatter of the target Pointer Source in `source/` to find its current path/URL and the `tracked_commit` (or previous hash).
   - Inspect the live, external resource to determine its current state (e.g., current `HEAD` commit).
   - Generate a diff or summarize the changelog between the `tracked_commit` and the current state.
...
3. Once the user has given direction, **write**:
   - **Update the Pointer Source:** Modify the body of the Pointer Source in `source/` to reflect the new snapshot/summary, and update the `tracked_commit` in the frontmatter to the new state.
